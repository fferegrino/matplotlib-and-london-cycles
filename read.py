...
def read_stations(beginning, end):
    ...
    _beginning = datetime(beginning.year, beginning.month, beginning.day) - timedelta(
        days=1
    )
    _end = datetime(end.year, end.month, end.day) + timedelta(days=1)

    dates = [
        (_beginning + timedelta(days=days)).strftime("%Y-%m-%d")
        for days in range((_end - _beginning).days + 1)
    ]
    ...
    all_data["query_time"] = pd.to_datetime(
        ...
        .dt.floor("15min", ambiguous=True)
    )

    # TODO: Fix division by zero, then we can delete this line
    all_data = all_data[all_data["docks"] != 0]

    all_data["occupancy"] = (all_data["docks"] - all_data["empty_docks"]) / all_data[
        "docks"
    ]
    ...
    if beginning and end:
        data_to_plot = all_data[
            (all_data["query_time"] >= beginning) & (all_data["query_time"] <= end)
        ]
    else:
        data_to_plot = all_data
    ...
    for bikepoint in data_to_plot["place_id"].unique():
        resampled = interpolate_bikepoint(
            data_to_plot[data_to_plot["place_id"] == bikepoint]
        )
